<?php
    ...
    function getTeamNameFromNum($intPitTeamNum){
        global $conScouting;
        $strQuery = "SELECT TeamName FROM tblAllFRCTeams WHERE TeamNum = ?";
      	// Check Connection
        if ($conScouting->connect_errno) {
            $blnError = "true";
            $strErrorMessage = $conScouting->connect_error;
        ...
        if ($conScouting->ping()) {
		 ...
		 $statScouting = $conScouting->prepare($strQuery);
		 // Bind Parameters
		 $statScouting->bind_param('ss', $intPitTeamNum, $strUserSessionID);
         $statScouting->execute();      
             $result = $statScouting->get_result();
             $myArray = array();
